...
export default async function AdminLeadsPage() {
  ...
  return (
    <div>
      <h1 style={{ fontSize: "1.6rem", fontWeight: 600, marginBottom: 8 }}>Leads</h1>
      <p style={{ color: "var(--ink-3)", fontSize: 13, marginBottom: 28 }}>
        Most recent 15 per table. Full filtering, status and assignment are Phase 2 (doc 15 §2).
      </p>

      <Section title="Project enquiries (BUILD)" rows={projectLeads} cols={["name", "email", "stage", "capability", "created_at"]} />
      <Section title="Growth enquiries (GROW)" rows={marketingLeads} cols={["name", "email", "services", "engagement_type", "created_at"]} />
      <Section
        title="Mentoring enquiries (TEACH)"
        rows={mentoringLeads}
        cols={["name", "email", "audience", "current_level", "is_minor", "created_at"]}
      />
    </div>
  );
}
...
function Section({ title, rows, cols }: { title: string; rows: Row[]; cols: string[] }) {
  return (
    <div style={{ marginBottom: 40 }}>
      <b style={{ display: "block", fontSize: 13.5, marginBottom: 12 }}>{title}</b>
      {rows.length === 0 ? (
